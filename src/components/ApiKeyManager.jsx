...
const ApiKeyManager = ({ isOpen, onClose, onApiKeysChange }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const retryTimer = setTimeout(() => {
        if (!stats && !loading) {
          console.log('Retrying API key stats load...')
          loadStats()
        }
      }, 2000)
  ...
  const loadStats = async () => {
    try {
      ...
      if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
        ...
        setError(`Failed to load API key stats: ${error.message}`)
      }
    } finally {
      setLoading(false)
    }
  ...
  const addApiKey = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (onApiKeysChange) onApiKeysChange() // Notify parent component
      } else {
        ...
        <div className="p-6">
          ...
          {loading && !stats && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-400">Đang tải thống kê API keys...</p>
            </div>
          )}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-400">API Key (hidden for security)</div>
                        </div>
                      </div>
