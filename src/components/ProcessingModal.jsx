...
const ProcessingModal = ({ isOpen, progress, onClose, onStop, partialResults }) => {
  if (!isOpen) return null

  const steps = [
    { id: 1, name: 'Gemini 1.5 Flash: Lấy duration', icon: Clock, result: partialResults?.duration },
    { id: 2, name: 'Gemini 1.5 Flash: Chia đoạn', icon: Circle, result: partialResults?.segments },
        ...
        <div className="p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-gray-400 mt-2">
              💡 Bạn có thể ẩn cửa sổ này và tiếp tục sử dụng ứng dụng
            </p>
          </div>
          ...
          <div className="mt-6 flex justify-center space-x-4">
            ...
            <button
              onClick={onStop}
              className="px-4 py-2 bg-red-600 bg-opacity-20 border border-red-500 rounded-lg hover:bg-opacity-30 transition-colors text-sm"
            >
              ⏹️ Dừng xử lý
            </button>
            ...
            <button
              onClick={onClose}
              className="glass-panel px-4 py-2 hover:bg-white hover:bg-opacity-10 transition-all duration-300 text-sm"
            >
              👁️ Ẩn cửa sổ này (tiếp tục xử lý nền)
            </button>
          </div>
        </div>
      </motion.div>
